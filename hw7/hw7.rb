...
class GeometryExpression  
  ...
  class GeometryValue 
    ...
    ## all subclass evaluate to itself 
    def eval_prog env
        self
    end
  ...
  class Point < GeometryValue
    ...
    # all val evaluate to self
    # no need to preprocess since not linesegment 
    ...
    # point intersect point is either point or no point
    def intersectPoint p
    ...
    def inbetween(v, end1, end2)
        ...
        (end1 - epsilon <= v && v <= end2 + epsilon) or (end2 - epsilon <= v) && (v <= end1 + epsilon)
    end
  ...
  class Line < GeometryValue
    ...
    def intersectLine line
        if real_close(line.m, m) # if parallel 
  ...
  class LineSegment < GeometryValue
    ...
    def intersectWithSegmentAsLineResult seg
        ...
        if real_close(x1, x2) # segment in same vertical line
    ...
    def preprocess_prog # only need to consider preprocess for linesegment 
        if real_close(x1, x2)
            if real_close(y1, y2)
                Point.new(x1, y1)
            elsif y1 > y2
