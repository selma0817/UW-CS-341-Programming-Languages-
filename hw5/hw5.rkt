...
(define(racketlist->mupllist racketlist)
  (if (null? racketlist) ;; if null, return empty list by eding value of list
       (aunit)
       (apair (car racketlist) (racketlist->mupllist (cdr racketlist))))) ;; apair is like cons in racket for making list

(define (mupllist->racketlist mupllist)
  (if (aunit? mupllist)
      null
      (cons (apair-e1 mupllist) (mupllist->racketlist (apair-e2 mupllist))))) ;; access e1 and e2 from this apair 
  ...
  (cond [(var? e) 
        ...
        [(mlet? e)
         (let* ([var-name (mlet-var e)]
               ...
               [new-env (cons (cons var-name var-val) env)])
           ;; extend env
         ...
         (let([clo (eval-under-env(call-funexp e) env)]
           ...
           (if (closure? clo)
               (let* ([clo-env (closure-env clo)]
                      [clo-fun (closure-fun clo)]
                      [clo-new-env (cons (cons (fun-formal clo-fun) argu) clo-env)]
                      [clo-new-env (if (fun-nameopt clo-fun)
                                      (cons (cons (fun-nameopt clo-fun) clo) clo-new-env)
                                      clo-new-env)])
                 (eval-under-env (fun-body clo-fun) clo-new-env))
...
(define (ifeq e1 e2 e3 e4)
  (mlet* (list (cons "_x" e1) (cons "_y" e2)) ;; if equal e3 else e4 check > and < 
